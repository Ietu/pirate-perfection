...
function TextBoxGui:_setup_buttons_panel( info_area, button_list, focus_button, only_buttons )
	...
	if has_buttons then
		local button_text_config = { x = 10, name = "button_text", font = tweak_data.menu.pd2_medium_font, font_size = tweak_data.menu.pd2_medium_font_size, vertical = "center", halign="right", layer = 2, wrap="true", word_wrap="true", blend_mode="add", color=Color.Patron } -- tweak_data.dialog.BUTTON_TEXT_COLOR
		local max_w = 0
		local max_h = 0
		...
		buttons_panel:set_right( info_area:w() - 10 )
			...
			local selected = buttons_panel:rect( { name="selected", blend_mode="add", color=tweak_data.screen_colors.Color.Patron, alpha=0.3 } )
