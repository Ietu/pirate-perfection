...
function MultiChoice:create_gui()
	...
	local text_panel = multi_choice:text( { name = "text", layer = 1, wrap = "true", word_wrap = "true", visible = true,
										font = pd2_medium_font, font_size = 20, color = Color.Patron,
										align="left", halign="left", vertical="center", valign="center", blend_mode = "add" } )
	self.text_panel = text_panel
	
	local arrow_left = multi_choice:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, color = Color.Patron, layer = 2 } )
	local arrow_right = multi_choice:bitmap( { texture = "guis/textures/menu_arrows", texture_rect = {0,0,24,24}, rotation = 180, color = Color.Patron, layer = 2 } )
...
function MultiChoice:update()
	...
	if arrow_left:inside( x, y ) then
		arrow_left:set_color( Color.Patron )
	...
	if arrow_right:inside( x, y ) then
		arrow_right:set_color( Color.Patron )
