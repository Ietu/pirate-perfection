...
local function car_update()
	...
	if not car_speedometer then
		...
		car_speedometer.lbl = car_speedometer.ws:panel():text{ name="lbl_speed" , x = speedometer_x * RenderSettings.resolution.x, y = speedometer_y * RenderSettings.resolution.y, text="", font=tweak_data.menu.pd2_large_font, font_size = speedometer_size * RenderSettings.resolution.x, color = Color.Patron, layer=2000 }
	end
