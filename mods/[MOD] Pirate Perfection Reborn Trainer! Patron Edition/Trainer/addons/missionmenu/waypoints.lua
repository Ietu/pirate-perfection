...
local function add_waypoint( unit )
	...
	if in_table( waypoint_items, tweak ) then
		M_hud:add_waypoint( tostring( unit:key() ), { icon = icon or 'wp_standard', distance = true, position = unit:position(), no_sync = true, present_timer = 0, state = "present", radius = 500, color = Color.Patron, blend_mode = "add" }  )
	end
end
