...
additional_option = function()
	local data = {
		{ text = tr.reduce_damage, plugin = 'less_damage', switch_back = true },
...
main_menu = function()
	local data = {
		...
		--{ text = tr.annoyer_mode, plugin_path = 'Trainer/addons/', plugin = 'nodelaytalk', switch_back = true },
		{},
	...
	if in_custody() then
		...
		insert(data, { text = tr.going_to_jail, callback = going_to_jail}) --Just add "_" before function to go to the jail with effect
	end
