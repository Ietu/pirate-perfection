	...
	if _toggleOverdrill then
		for k,v in pairs(managers.interaction._interactive_units) do
			if v:interaction().tweak_data == 's_cube' then
				...
				then
					managers.hud:add_waypoint( 'rtile_'..k, { icon = 'wp_target', distance = false, position = v:position()+Vector3(-25,0,0), no_sync = true, present_timer = 0, state = "present", radius = 10000, color = Color.Patron, blend_mode = "add" }  )
				else
					managers.hud:add_waypoint( 'wtile_'..k, { icon = 'wp_target', distance = false, position = v:position()+Vector3(-25,0,0), no_sync = true, present_timer = 0, state = "present", radius = 10000, color = Color.Patron, blend_mode = "add" }  )
				end
			end
		end
	end
...
main = function()
	...
	if current_level == "red2" and Global.game_settings.difficulty == "easy_wish" or Global.game_settings.difficulty == "overkill_290" or Global.game_settings.difficulty == "sm_wish" and is_server then
